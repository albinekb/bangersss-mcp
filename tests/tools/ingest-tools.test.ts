...
async function findMp3Files(directory: string): Promise<string[]> {
  ...
  return fg('**/*.mp3', { cwd: directory, absolute: true, onlyFiles: true })
}
...
describe('Ingest tools integration', () => {
  ...
  const testFiles = [
    ...
    },
    {
      ...
      tags: { title: 'Levels', artist: 'Avicii', genre: 'EDM', bpm: '126' },
    },
    { name: 'unknown_track.mp3', tags: { title: 'Unknown Track' } }, // missing artist/genre
    {
      name: 'Charlotte de Witte - Overdrive.mp3',
      tags: {
        title: 'Overdrive',
  ...
  beforeAll(async () => {
    ...
    const existing = createMinimalMp3WithTags({
      title: 'Levels',
      artist: 'Avicii',
      genre: 'EDM',
    })
    ...
    await fs.writeFile(
      path.join(libraryDir, 'EDM', 'Avicii', 'Levels.mp3'),
      existing,
    )
  })
  ...
  describe('check_duplicates logic', () => {
    it('detects duplicates by artist+title', async () => {
      ...
      for (const f of libFiles) {
        try {
          ...
          if (tags.artist && tags.title) {
            tagIndex.set(
              `${tags.artist.toLowerCase()}|${tags.title.toLowerCase()}`,
              f,
            )
          }
        } catch {
      ...
      // Check incoming
      const incomingFiles = await findMp3Files(incomingDir)
    ...
    it('detects duplicates by filename', async () => {
      const libFiles = await findMp3Files(libraryDir)
      const filenameIndex = new Set(
        libFiles.map((f) => path.basename(f).toLowerCase()),
      )

      const incomingFiles = await findMp3Files(incomingDir)
      const filenameDups = incomingFiles.filter((f) =>
        filenameIndex.has(path.basename(f).toLowerCase()),
      )
  ...
  describe('plan creation for ingest', () => {
    ...
    it('executes plan in dry mode without moving files', async () => {
      ...
      const result = await context.planManager.executePlan(plan.id, {
        dryMode: true,
      })
