...
describe('commitOperations', () => {
  ...
  it('commits a write operation to real fs', async () => {
    ...
    const result = await commitOperations([
      { type: 'write', path: filePath, data: Buffer.from('committed!'), timestamp: new Date().toISOString() },
    ]);
  ...
  it('commits rename operation', async () => {
    ...
    const result = await commitOperations([
      { type: 'rename', path: fromPath, from: fromPath, to: toPath, timestamp: new Date().toISOString() },
    ]);
  ...
  it('reports failures without aborting other ops', async () => {
    ...
    const result = await commitOperations([
      { type: 'rename', path: '/nonexistent', from: '/nonexistent', to: '/also-nope', timestamp: new Date().toISOString() },
      { type: 'write', path: goodPath, data: Buffer.from('still works'), timestamp: new Date().toISOString() },
    ]);
