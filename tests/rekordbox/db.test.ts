...
import {
  findRekordboxDb,
  DEFAULT_REKORDBOX_DB_PATH,
  openRekordboxDb,
  closeRekordboxDb,
} from '../../src/rekordbox/db.js'
...
describe('Rekordbox DB', () => {
  it.skipIf(!hasRealDb)(
    'findRekordboxDb detects the real DB on this machine',
    () => {
  ...
  it.skipIf(!hasRealDb)(
    'openRekordboxDb auto-detects and opens with decryption',
    async () => {
      let db
      try {
        db = await openRekordboxDb()
        const tracks = db.loadTracks(1)
        expect(tracks).toBeDefined()
        expect(tracks!.count).toBeGreaterThan(0)
      } finally {
        if (db) closeRekordboxDb(db)
      }
    },
  )

  it('throws on non-existent path', async () => {
    await expect(openRekordboxDb('/nonexistent/master.db')).rejects.toThrow()
  })
})
...
describe('Rekordbox DB with mock unencrypted SQLite', () => {
  ...
  it('creates and queries a mock Rekordbox DB schema', async () => {
    ...
    db.exec(`
      CREATE TABLE djmdContent (
        ...
        GenreID TEXT, BPM REAL, Rating INTEGER, Key INTEGER,
        Duration REAL, FolderPath TEXT, FileNameL TEXT, FilePath TEXT
      );
      ...
      CREATE TABLE djmdPlaylist (
        ID TEXT PRIMARY KEY, Name TEXT, ParentID TEXT, Seq INTEGER, Attribute INTEGER
      );
      CREATE TABLE djmdSongPlaylist (
        ID TEXT PRIMARY KEY, ContentID TEXT, PlaylistID TEXT, TrackNo INTEGER
      );
      CREATE TABLE djmdCue (
        ID TEXT PRIMARY KEY, ContentID TEXT, InMsec REAL, OutMsec REAL,
        Kind INTEGER, Color INTEGER, Hotcue INTEGER, Comment TEXT,
        ActiveLoop INTEGER, BeatLoopSize INTEGER
      );
    `)

    db.prepare('INSERT INTO djmdArtist (ID, Name) VALUES (?, ?)').run(
      'a1',
      'DJ Snake',
    )
    db.prepare('INSERT INTO djmdArtist (ID, Name) VALUES (?, ?)').run(
    ...
    db.prepare('INSERT INTO djmdGenre (ID, Name) VALUES (?, ?)').run(
      'g1',
      'House',
    )
    db.prepare('INSERT INTO djmdGenre (ID, Name) VALUES (?, ?)').run(
    ...
    db.prepare(
      'INSERT INTO djmdContent (ID, Title, ArtistID, GenreID, BPM, Rating, Duration, FilePath) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    db.prepare(
      'INSERT INTO djmdCue (ID, ContentID, InMsec, Kind, Hotcue, Comment) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    const trackCount = (
      db.prepare('SELECT COUNT(*) as count FROM djmdContent').get() as {
        count: number
      }
    ).count
  ...
  it('searches tracks by genre join', () => {
    const rows = db
      ...
      FROM djmdContent c LEFT JOIN djmdGenre g ON c.GenreID = g.ID
      WHERE g.Name LIKE ?
    `,
      )
      .all('%EDM%')
  ...
  it('searches tracks by BPM range', () => {
    const rows = db
      .prepare('SELECT * FROM djmdContent WHERE BPM >= ? AND BPM <= ?')
      .all(125, 128)
  ...
  it('lists playlists', () => {
    const playlists = db
      .prepare('SELECT * FROM djmdPlaylist WHERE Attribute = 1')
      .all()
  ...
  it('gets playlist tracks with join', () => {
    const tracks = db
      ...
      FROM djmdSongPlaylist sp JOIN djmdContent c ON sp.ContentID = c.ID
  ...
  it('gets cue points', () => {
    const cues = db
      .prepare('SELECT * FROM djmdCue WHERE ContentID = ?')
      .all('c1')
