...
describe('Plan Zod schemas', () => {
  it('validates a rename_file operation', () => {
    const op = { type: 'rename_file', status: 'pending', from: '/a.mp3', to: '/b.mp3' };
    expect(OperationSchema.parse(op)).toEqual(op);
  });

  it('validates a write_tags operation', () => {
    const op = { type: 'write_tags', status: 'done', path: '/a.mp3', tags: { artist: 'Test' } };
    expect(OperationSchema.parse(op)).toEqual(op);
  });
  ...
  it('rejects invalid status', () => {
    const op = { type: 'rename_file', status: 'invalid', from: '/a.mp3', to: '/b.mp3' };
    expect(() => OperationSchema.parse(op)).toThrow();
  });
  ...
  it('validates a full plan', () => {
    const plan = {
      ...
      baseDirectory: '/music',
      operations: [
        { type: 'rename_file', status: 'pending', from: '/a.mp3', to: '/b.mp3' },
