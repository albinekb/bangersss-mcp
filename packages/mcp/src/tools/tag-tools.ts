      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error reading tags from ${filePath}: ${message}`,
            },
          ],
        }
      }
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error batch reading tags: ${message}`,
            },
          ],
        }
      }
  ...
  server.tool(
    ...
    'Write metadata tags to an audio file via the overlay filesystem (non-destructive until committed).',
    {
      ...
      tags: z
        ...
        .describe(
          'Tag fields to write (e.g. { title: "...", artist: "...", bpm: 128 })',
        ),
    },
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error writing tags to ${filePath}: ${message}`,
            },
          ],
        }
      }
      ...
      try {
              ...
              text: JSON.stringify(
                {
                  ...
                  suggestions: missing.map(
                    (f) => `Field "${f}" is empty and could be populated.`,
                  ),
                },
                null,
                2,
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error analyzing tags for ${filePath}: ${message}`,
            },
          ],
        }
      }
