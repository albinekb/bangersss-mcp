...
describe('Engine DJ DB with mock SQLite', () => {
  ...
  it('creates and queries a mock Engine DJ schema', async () => {
    ...
    // Insert test data
    db.prepare(
      'INSERT INTO Track (id, title, artist, genre, bpm, duration, path, filename) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      1,
      'Track One',
      'Artist A',
  ...
  it('searches tracks by artist', () => {
    const rows = db
      .prepare('SELECT * FROM Track WHERE artist LIKE ?')
      .all('%Artist A%')
  ...
  it('searches tracks by BPM range', () => {
    const rows = db
      .prepare('SELECT * FROM Track WHERE bpm >= ? AND bpm <= ?')
      .all(125, 130)
    expect(rows).toHaveLength(2)
  })
  ...
  it('searches tracks by genre', () => {
    const rows = db
      .prepare('SELECT * FROM Track WHERE genre LIKE ?')
      .all('%Techno%')
  ...
  it('adds track to crate', () => {
    ...
    const count = (
      db
        .prepare(
          'SELECT COUNT(*) as count FROM CrateTrackList WHERE crateId = 1',
        )
        .get() as { count: number }
    ).count
    expect(count).toBe(3)
  ...
  it('creates a new crate', () => {
    db.prepare('INSERT INTO Crate (title, path) VALUES (?, ?)').run(
      'New Crate',
      'Root;New Crate;',
    )
    const crates = db.prepare('SELECT * FROM Crate ORDER BY title').all()
