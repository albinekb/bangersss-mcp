...
export function registerScanTools(server: McpServer, _context: ServerContext): void {
  server.tool(
    'scan_directory',
    'Scan a directory for audio files using fast-glob. Returns a list of discovered audio file paths.',
    {
      ...
      recursive: z.boolean().optional().default(true).describe('Whether to scan subdirectories recursively'),
      extensions: z.array(z.string()).optional().describe('File extensions to include (e.g. [".mp3", ".flac"]). Defaults to all supported audio formats.'),
    },
    async ({ path: dirPath, recursive, extensions }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error scanning directory: ${message}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    'List audio files in a directory with optional filtering by glob pattern and sorting.',
    {
      ...
      pattern: z.string().optional().describe('Glob pattern to filter files (e.g. "*.mp3")'),
      sortBy: z.enum(['name', 'date', 'size']).optional().default('name').describe('Sort order for results'),
    },
