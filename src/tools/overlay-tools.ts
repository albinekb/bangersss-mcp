...
export function registerOverlayTools(
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error getting pending changes: ${message}`,
            },
          ],
        }
      }
    ...
    async ({ path: filePath }) => {
      try {
        ...
        const relevant = operations.filter(
          (op) =>
            op.path === filePath || op.from === filePath || op.to === filePath,
        )
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error previewing change: ${message}`,
            },
          ],
        }
      }
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error committing changes: ${message}`,
            },
          ],
        }
      }
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error discarding changes: ${message}`,
            },
          ],
        }
      }
  ...
  server.tool(
    ...
    'Commit only overlay changes that affect the specified file paths.',
    {
      paths: z
        .array(z.string())
        .describe('Array of absolute file paths to commit'),
    },
    async ({ paths }) => {
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error committing selective changes: ${message}`,
            },
          ],
        }
      }
