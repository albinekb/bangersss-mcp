...
export function registerIngestTools(
  ...
  server.tool(
      ...
      'overview, then drill down with groupBy or limit/offset.',
    {
      ...
      summaryOnly: z
        ...
        .default(false)
        .describe(
          'Only return aggregate stats (format counts, tag coverage, date ranges) — no file list',
        ),
      skipTags: z
        .boolean()
        .optional()
        .default(false)
        .describe(
          'Skip tag reading for faster scanning — returns file info without tag data or tag coverage stats',
        ),
      groupBy: z
        .enum(['artist', 'genre', 'date', 'format', 'folder'])
        .optional()
        .describe(
      ...
      limit: z
        ...
        .default(50)
        .describe(
          'Max number of files to return in the tracks list (default 50)',
        ),
      offset: z
        .number()
        .optional()
      ...
      sortBy: z
        .enum(['name', 'date', 'size', 'artist', 'bpm'])
        .optional()
        .default('date')
      ...
      filter: z
        .object({
          artist: z
            .string()
            .optional()
            .describe('Filter by artist (partial match, case-insensitive)'),
          genre: z
            .string()
            .optional()
            .describe('Filter by genre (partial match, case-insensitive)'),
          format: z
            .string()
            .optional()
            .describe('Filter by extension, e.g. ".mp3" or ".wav"'),
          hasTag: z
            .enum(['title', 'artist', 'genre', 'bpm', 'key'])
            .optional()
    ...
    async ({
      ...
      try {
        ...
        for (const { filePath } of walkedFiles) {
          ...
          if (!dateRange.earliest || modified < dateRange.earliest)
            dateRange.earliest = modified
          if (!dateRange.latest || modified > dateRange.latest)
            dateRange.latest = modified
        ...
        if (filter) {
          filtered = tracks.filter((t) => {
            ...
            if (filter.genre && t.tags?.genre) {
              if (
                !t.tags.genre.toLowerCase().includes(filter.genre.toLowerCase())
              )
                return false
            ...
            if (filter.dateBefore && t.modified > filter.dateBefore)
              return false
            return true
          })
        }
        ...
        if (groupBy) {
          ...
          for (const t of filtered) {
            ...
            if (!groups[key])
              groups[key] = { count: 0, totalSize: 0, files: [] }
            groups[key].count++
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error scanning incoming folder: ${message}`,
            },
          ],
        }
      }
  ...
  server.tool(
      ...
      'Optionally suggests where each file should go in the library.',
    {
      paths: z
        .array(z.string())
        .describe('Absolute paths to audio files to analyze'),
      detectBpm: z
        .boolean()
        .optional()
        .default(false)
        .describe('Run BPM detection (requires ffmpeg, slower)'),
      libraryRoot: z
        .string()
        .optional()
        .describe('Library root path — if set, suggests destination paths'),
      organizationTemplate: z
        .string()
        .optional()
        .default('{genre}/{artist}/{title}')
        .describe(
          'Template for library organization, e.g. "{genre}/{artist}/{title}"',
        ),
    },
    async ({
      paths: filePaths,
      ...
      try {
        ...
        for (const filePath of filePaths) {
          ...
          if (detectBpm) {
            try {
              ...
              if (result.confidence < 0.5) {
                issues.push(
                  `Low BPM confidence (${Math.round(result.confidence * 100)}%)`,
                )
              }
              if (!tags?.bpm && result.bpm > 0) {
                ...
                issues.push(
                  `BPM mismatch: tag says ${tags.bpm}, detected ${result.bpm}`,
                )
              }
            } catch (err) {
              issues.push(
          ...
          if (libraryRoot && tags) {
            suggestedPath = buildLibraryPath(
              libraryRoot,
              tags,
              organizationTemplate,
              ext,
              ...
              text: JSON.stringify(
                {
                  ...
                  totalIssues,
                  filesWithIssues: results.filter((r) => r.issues.length > 0)
                    .length,
                  cleanFiles: results.filter((r) => r.issues.length === 0)
                    .length,
                  results,
                },
                null,
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error analyzing files: ${message}`,
            },
          ],
        }
      }
  ...
  server.tool(
      ...
      'filename similarity, and file size. Returns matches and near-matches.',
    {
      incomingPaths: z
        .array(z.string())
        .describe('Paths to incoming files to check'),
      libraryPath: z
        .string()
        .describe('Absolute path to the existing music library'),
    },
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error checking duplicates: ${message}`,
            },
          ],
        }
      }
  ...
  server.tool(
      ...
      'is written until you commit. Creates a Plan for the ingest that can be exported/resumed.',
    {
        ...
        .array(
          z.object({
            ...
            destinationPath: z
              .string()
              .optional()
              .describe('Override destination path (if not using template)'),
            tagsToWrite: z
              .record(z.string(), z.unknown())
              .optional()
              .describe('Tags to write before moving'),
      ...
      planName: z
        ...
        .optional()
        .default('Ingest')
        .describe('Name for the ingest plan'),
    },
    async ({ files, libraryRoot, organizationTemplate, planName }) => {
      try {
        ...
        for (const file of files) {
          ...
          if (file.tagsToWrite && Object.keys(file.tagsToWrite).length > 0) {
            context.planManager.addOperation(
              plan.id,
              createWriteTagsOp(file.sourcePath, file.tagsToWrite),
            )
            operations.push(
              `write tags: ${Object.keys(file.tagsToWrite).join(', ')}`,
            )
          }
          ...
          if (file.bpm !== undefined) {
            context.planManager.addOperation(
              plan.id,
              createSetBpmOp(file.sourcePath, file.bpm),
            )
          ...
          context.planManager.addOperation(
            plan.id,
            createRenameOp(file.sourcePath, dest),
          )
        ...
        return {
          content: [
            { type: 'text' as const, text: `Error staging ingest: ${message}` },
          ],
        }
      }
    },
  ...
  server.tool(
      ...
      'check_duplicates, and stage_ingest into a single call.',
    {
      incomingPath: z
        .string()
        .describe('Folder with new music to ingest (e.g. ~/Downloads)'),
    ...
    async ({
      ...
      try {
        ...
        const incomingTagMap = await batchReadTags(incomingFiles, {
          concurrency: 8,
        })
        ...
        for (const track of toIngest) {
          ...
          context.planManager.addOperation(
            plan.id,
            createRenameOp(track.path, dest),
          )
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error in quick ingest: ${message}`,
            },
          ],
        }
      }
