...
export function registerFileTools(server: McpServer, context: ServerContext): void {
  server.tool(
    ...
    async ({ from, to }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error renaming file: ${message}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      directory: z.string().describe('Absolute path to the directory containing files'),
      pattern: z.string().describe('Glob pattern to match files (e.g. "*.mp3")'),
      template: z.string().describe('Rename template using tag placeholders, e.g. "{artist} - {title}"'),
    },
    async ({ directory, pattern, template }) => {
      try {
        const files = await fg(`${directory}/${pattern}`, {
          absolute: true,
          onlyFiles: true,
        });
        ...
        for (const filePath of audioFiles) {
          try {
            ...
            for (const [key, value] of Object.entries(replacements)) {
              newName = newName.replace(new RegExp(`\\{${key}\\}`, 'gi'), sanitizeFilename(value));
            }
            ...
            const message = err instanceof Error ? err.message : String(err);
            results.push({ from: filePath, to: '', success: false, error: message });
          }
        }
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text' as const, text: `Error in batch rename: ${message}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      directory: z.string().describe('Absolute path to the directory containing files to organize'),
      template: z.string().describe('Folder structure template, e.g. "{genre}/{artist}/{title}"'),
    },
    async ({ directory, template }) => {
      try {
        ...
        for (const filePath of audioFiles) {
          try {
            ...
            if (lastSegment.includes('.') || template.includes('{title}')) {
              ...
              newPath = path.join(directory, newRelativePath, path.basename(filePath));
            }
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text' as const, text: `Error organizing files: ${message}` }],
        };
      }
    },
  );
